...
DISQUE DUR (AHCI / SATA) :
---------------------------
  Commande le controleur AHCI directement via PCI, détecte les disques
  SATA et monte les partitions FAT32. Ne fonctionne que sur du matériel
  réel (pas d'émulation).

  disk:init         ( -- bool )     — Initialise AHCI + monte FAT32
  disk:ls <chemin>  ( -- )          — Liste le répertoire (ex: disk:ls /)
  disk:read <fich>  ( addr -- n )   — Lit fichier dans la mémoire Forth
  disk:write <fich> ( addr len -- ) — Écrit la mémoire vers le disque
                                       (écriture non implémentée pour l'instant)
